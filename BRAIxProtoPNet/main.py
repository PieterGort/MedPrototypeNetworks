...
from dataset import create_polyp_datasets, get_WRsampler_dataset

# book keeping namings and code
from settings import base_architecture, img_size, prototype_shape, num_classes, \
                     prototype_activation_function, add_on_layers_type, experiment_run, \
                    log_dir, randseeddata, randseedother, coefs, dataset, image_folder, num_classes

# number of training epochs, number of warm epochs, push start epoch, push epochs
from settings import train_batch_size, test_batch_size, train_push_batch_size, usevalidation, num_train_epochs, num_stage1_epochs, num_stage2_epochs, kfold, num_workers                               
...
def load_model(net, stage1_optimizer, stage2_optimizer, joint_optimizers, path):
    if path != '':
        ...
        joint_optimizers[0].load_state_dict(checkpoint['optimizer_joint_backbone'])
        joint_optimizers[1].load_state_dict(checkpoint['optimizer_joint_lastlayer'])
        ...
        epoch_stage2 = 0
        epoch_joint = 0
    return net, stage1_optimizer, stage2_optimizer, joint_optimizers, start_epoch, epoch_stage1, epoch_stage2, epoch_joint
...
def main():
    ...
    # Combine settings and arguments into a single dictionary
    config = {**vars(args), **{k: v for k, v in globals().items() if k in [
        'base_architecture', 'img_size', 'prototype_shape', 'num_classes', 
        'prototype_activation_function', 'add_on_layers_type', 'experiment_run', 
        'log_dir', 'randseeddata', 'randseedother', 'coefs', 'dataset', 'image_folder', 
        'train_batch_size', 'test_batch_size', 'train_push_batch_size', 'usevalidation', 
        'num_train_epochs', 'num_stage1_epochs', 'num_stage2_epochs', 'kfold', 'num_workers', 
        'joint_optimizer_lrs', 'joint_lr_step_size', 'stage1_optimizer_lrs', 'stage2_optimizer_lrs', 
        'last_layer_optimizer_lr', 'lrscheduler', 'lrdecay_gamma'
    ]}}

    wandb.init(project="BRAIxProtoPNetxx", name="PPNet-Braixx", config=config)
    ...
    train_transform = transforms.Compose([
        ...
        transforms.RandomResizedCrop(img_size, scale=(0.98, 1.02), antialias=True),
        transforms.RandomAffine(degrees=180, translate=(0.25, 0.25), scale=(0.9, 1.1)),
    ...
    train_dataset, val_dataset, test_dataset, train_push_dataset = create_polyp_datasets(image_folder, train_transform, val_transform, push_transform, k=kfold)

    train_loader = DataLoader(train_dataset, batch_size=train_batch_size, num_workers=num_workers, sampler=get_WRsampler_dataset(train_dataset), shuffle=False, pin_memory=True)
    val_loader = DataLoader(val_dataset, batch_size=test_batch_size, num_workers=num_workers, shuffle=False, pin_memory=True)
    test_loader = DataLoader(test_dataset, batch_size=test_batch_size, num_workers=num_workers, shuffle=False, pin_memory=True)
    train_push_loader = DataLoader(train_push_dataset, batch_size=train_push_batch_size, num_workers=num_workers, sampler=get_WRsampler_dataset(train_push_dataset), shuffle=False, pin_memory=True)
    ...
    for epoch in range(start_epoch, num_train_epochs+1):
        ...
        if usevalidation:
            ...
            wandb.log({
                'train_loss': info_train['avg_loss'],
                'val_loss': info_val['avg_loss'],
                'train_auc': info_train['auc_both'],
                'val_auc': info_val['auc_both'],
                'train_acc': info_train['correct_both'] / info_train['total_image'],
                'val_acc': info_val['correct_both'] / info_val['total_image'],
                'val_f1': info_val['f1_score'],
                'epoch': epoch,	
            }, step=epoch)
        else:
