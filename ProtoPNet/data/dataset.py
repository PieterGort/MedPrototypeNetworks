...
def get_df_vindrmammo(image_dir, csv_path, n_classes, bbox_experiment=False):
...
def show_image(tensor, title="Transformed Image"):
    """Function to display a transformed tensor as an image."""
    image = tensor.clone().detach()
    image = image.cpu().numpy().transpose(1, 2, 0)
    mean = torch.tensor([0.485, 0.456, 0.406])
    std = torch.tensor([0.229, 0.224, 0.225])
    image = std * image + mean  # Un-normalize
...
def main():
    ...
    train_transform = v2.Compose([
        ...
        v2.Resize((1536, 768), antialias=True),
        #v2.RandomResizedCrop((1536, 768), scale=(0.95, 1.0), antialias=True),
        v2.RandomHorizontalFlip(),
        v2.RandomVerticalFlip(),
        v2.RandomAffine(degrees=20, translate=(0.10, 0.10), scale=(0.95, 1.05)),
        v2.ToDtype(torch.float32, scale=True),
        v2.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
    ])

    # Load an image (update with your image path)
    image_path = r'c:\Users\PCpieter\Documents\vscode\mscthesis\VinDrMammo\Processed_Images\00a7a306c763ab5f0cb6a846825b7d04\2ce50597d3a7711d30b9ec3a0aa25623.png'
    ...
    show_image(transformed_img, title="Transformed Image")
