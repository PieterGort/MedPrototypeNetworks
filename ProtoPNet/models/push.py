...
def update_prototypes_on_batch(search_batch_input,
    ...
    for j in range(n_prototypes):
        ...
        if batch_min_proto_dist_j < global_min_proto_dist[j]:
            ...
            print("Image sizes used for dsize", original_img_size[1], original_img_size[0])
            upsampled_act_img_j = cv2.resize(proto_act_img_j, dsize=(original_img_size[1], original_img_size[0]),
                                             interpolation=cv2.INTER_CUBIC)
            
            proto_bound_j = find_high_activation_crop(upsampled_act_img_j, percentile=activation_percentile)
...
def main():
    ...
    # import the utils path using sys
    sys.path.append('utils')
    # Define a simple Dataset class that returns random images of the specified size
    class RandomImageDataset(Dataset):
        ...
        def __getitem__(self, idx):
            # Generate a random image and a random class label (let's assume 10 classes)
            image = np.random.rand(*self.image_size).astype(np.float32)  # Random float values in [0, 1]
            label = np.random.randint(0, 10)  # Random integer label
            return torch.from_numpy(image), label

    # Initialize the dataset and dataloader
    image_size = (3, 1536, 768)
    num_images = 100  # number of images in the dataset
    batch_size = 10   # number of images per batch

    dataset = RandomImageDataset(num_images, image_size)
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)

    # Define a dummy model and preprocess function for demonstration
    class DummyPrototypeNetwork:
        def __init__(self):
            self.module = DummyModule()

        def eval(self):
            pass  # Assume the model is always in evaluation mode for simplicity

    class DummyModule:
        def __init__(self):
            self.prototype_shape = (10, 3, 3, 3)  # Example shape
            self.num_prototypes = 10
            self.prototype_vectors = torch.randn(10, 3, 3, 3)  # Random initial prototypes
            self.num_classes = 10
            self.proto_layer_rf_info = None  # Simplified: no receptive field info provided

        def push_forward(self, input):
            # Just return the input and some dummy distances for demonstration
            return input, torch.abs(input - self.prototype_vectors.mean(dim=0, keepdim=True))

    def dummy_preprocess_function(input):
        # No preprocessing in this example
        return input

    # Initialize the dummy model
    prototype_network_parallel = DummyPrototypeNetwork()

    # Run the push_prototypes function
    push_prototypes(dataloader=dataloader,
                    prototype_network_parallel=prototype_network_parallel,
                    preprocess_input_function=dummy_preprocess_function,
                    root_dir_for_saving_prototypes='path_to_save',
                    epoch_number=1)
