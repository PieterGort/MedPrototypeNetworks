...
class ImagePatchWithFilename:

    def __init__(self, patch, label, distance, filename,
...
class ImagePatchInfoWithFilename:

    def __init__(self, label, distance, filename):
        self.label = label
        self.negative_distance = -distance
        self.filename = filename
...
def find_k_nearest_patches_to_prototypes_with_filenames(dataloader, prototype_network_parallel, k=5,
                                         preprocess_input_function=None, full_save=False,
                                         root_dir_for_saving_images='./nearest', log=print,
                                         prototype_activation_function_in_numpy=None, activation_percentile=94):
    ...
    heaps = []
    bboxes_all_prototypes = []
    for _ in range(n_prototypes):
        heaps.append([])

    for idx, (search_batch_input, search_y, search_filenames) in enumerate(dataloader):
        ...
        for img_idx, distance_map in enumerate(proto_dist_):
            for j in range(n_prototypes):
                ...
                if full_save:
                    ...
                    closest_patch = ImagePatchWithFilename(patch=closest_patch, label=search_y[img_idx],
                                               distance=closest_patch_distance_to_prototype_j,
                                               filename=search_filenames[img_idx], original_img=original_img,
                                               act_pattern=act_pattern, patch_indices=patch_indices)
                else:
                    closest_patch = ImagePatchInfoWithFilename(label=search_y[img_idx],
                                                   distance=closest_patch_distance_to_prototype_j,
                                                   filename=search_filenames[img_idx])

                if len(heaps[j]) < k:
                    heapq.heappush(heaps[j], closest_patch)
                else:
                    replaced = heapq.heappushpop(heaps[j], closest_patch)
        # if idx == 0:
    ...
    for j in range(n_prototypes):
        ...
        if full_save:
            ...
            for i, patch in enumerate(heaps[j]):
                ...
                plt.imsave(fname=os.path.join(dir_for_saving_images, 'nearest-' + str(i+1) + '_original.png'),
                           arr=patch.original_img, vmin=0.0, vmax=1.0)
                
                # Changed this to accept non-square images
                img_size = patch.original_img.shape[0], patch.original_img.shape[1]

                upsampled_act_pattern = cv2.resize(patch.act_pattern, dsize=(img_size[1], img_size[0]), interpolation=cv2.INTER_CUBIC)
                ...
                plt.imsave(fname=os.path.join(dir_for_saving_images, 'nearest-' + str(i+1) + '_original_with_heatmap.png'),
                ...
                if patch.patch.shape[0] != img_size or patch.patch.shape[1] != img_size:
                    ...
                    plt.imsave(fname=os.path.join(dir_for_saving_images, 'nearest-' + str(i+1) + '_receptive_field.png'),
                    ...
                    plt.imsave(fname=os.path.join(dir_for_saving_images, 'nearest-' + str(i+1) + '_receptive_field_with_heatmap.png'),
                               arr=overlayed_patch, vmin=0.0, vmax=1.0)

                high_act_patch_indices = find_high_activation_crop(upsampled_act_pattern, percentile=activation_percentile) # formerly 99.8 percentile
                ...
                plt.imsave(fname=os.path.join(dir_for_saving_images, 'nearest-' + str(i+1) + '_high_act_patch.png'),
                           arr=high_act_patch, vmin=0.0, vmax=1.0)
                imsave_with_bbox(fname=os.path.join(dir_for_saving_images, 'nearest-' + str(i+1) + '_high_act_patch_in_original_img.png'),
                                 img_rgb=patch.original_img, bbox_height_start=high_act_patch_indices[0],
                                 bbox_height_end=high_act_patch_indices[1], bbox_width_start=high_act_patch_indices[2],
                                 bbox_width_end=high_act_patch_indices[3], color=(0, 255, 255))
                
                xmin = high_act_patch_indices[2]
                xmax = high_act_patch_indices[3]
                ymin = high_act_patch_indices[0]
                ymax = high_act_patch_indices[1]

                bboxes.append([xmin, ymin, xmax, ymax, j, i])
                labels.append(patch.label)
                filenames.append(patch.filename)
                
            np.save(os.path.join(dir_for_saving_images, 'class_id.npy'), labels)
            np.save(os.path.join(dir_for_saving_images, 'filenames.npy'), filenames)
            np.save(os.path.join(dir_for_saving_images, 'bboxes.npy'), np.array(bboxes))

    labels_all_prototype = np.array([[patch.label for patch in heaps[j]] for j in range(n_prototypes)])
    filenames_all_prototype = np.array([[patch.filename for patch in heaps[j]] for j in range(n_prototypes)])

    if full_save:
        np.save(os.path.join(root_dir_for_saving_images, 'full_class_id.npy'), labels_all_prototype)
        np.save(os.path.join(root_dir_for_saving_images, 'full_filenames.npy'), filenames_all_prototype)
        np.save(os.path.join(dir_for_saving_images, 'bboxes.npy'), np.array(bboxes))

    bboxes_all_prototypes.append(bboxes)
    ...
    return labels_all_prototype, filenames_all_prototype, bboxes_all_prototypes
